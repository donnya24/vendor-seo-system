...
<!-- ===== Sinkron margin dengan sidebar (offset 15rem = w-60) ===== -->
<style>
  :root { --sidebar-offset: 0px; }
  @media (min-width: 768px){
    #pageWrap{ margin-left: var(--sidebar-offset); transition: margin-left .3s ease; }
  }
  /* Paksa konten halaman penuh, abaikan container global yang mungkin memberi max-width & mx-auto */
  #pageWrap .page-inner{
  ...
  /* Hilangkan scrollbar horizontal yang tidak perlu di area tabel */
  #pageWrap .table-wrap{ width: 100%; overflow-x: auto; }
  #pageWrap table{ width: 100%; /* biar kolom menyesuaikan */ }
</style>
<script>
  // Update CSS var --sidebar-offset berdasarkan Alpine store
  (function(){
    function applyOffset(){
      try{
        const L = window.Alpine?.store('layout');
        const open = !!(L && L.sidebarOpen && L.isDesktop);
        document.documentElement.style.setProperty('--sidebar-offset', open ? '15rem' : '0px'); // w-60
      }catch(e){}
    }
    ...
    // fallback polling tipis untuk jaga-jaga toggle via event lain
    const _t = setInterval(applyOffset, 150);
    window.addEventListener('beforeunload', ()=>clearInterval(_t));
  })();
</script>

<div id="pageWrap" class="flex-1 flex flex-col overflow-hidden" x-data="{ openFilter: false }">
  <!-- Header Section -->
  <header class="bg-white shadow-md z-20 sticky top-0">
    <div class="page-inner px-4 sm:px-6 py-4 flex items-center justify-between">
      <div>
  ...
  <main class="flex-1 overflow-y-auto bg-gray-50">
    <div class="page-inner px-4 sm:px-6 py-6">
      ...
      <!-- Leads Table -->
      <div class="bg-white rounded-xl shadow-sm overflow-hidden animate-slide-up w-full">
        <div class="table-wrap">
          <!-- Hapus table-fixed agar kolom tidak memaksa lebar & memicu scrollbar -->
          ...
          <div class="px-6 py-4 bg-gray-50 border-t border-gray-200 flex items-center justify-between">
            ...
            <div class="flex space-x-2">
              <?php if ($pager->hasPreviousPage()): ?>
                <a href="<?= $pager->getPreviousPageURI() ?>" class="px-3 py-1.5 border border-gray-300 rounded-md text-sm font-medium text-gray-700 hover:bg-gray-50">Sebelumnya</a>
              <?php endif; ?>
              <?php if ($pager->hasNextPage()): ?>
                <a href="<?= $pager->getNextPageURI() ?>" class="px-3 py-1.5 border border-gray-300 rounded-md text-sm font-medium text-gray-700 hover:bg-gray-50">Selanjutnya</a>
              <?php endif; ?>
            </div>
          </div>
        <?php endif; ?>
