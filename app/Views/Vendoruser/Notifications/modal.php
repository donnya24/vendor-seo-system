...
<div x-show="notifModal"
  ...
  <div class="bg-white rounded-lg shadow-lg w-full max-w-lg mx-4"
        ...
        <?php foreach ($notifications as $n): ?>
          <div class="p-4 flex justify-between items-center hover:bg-gray-50">
            ...
            <div class="flex items-center gap-2 shrink-0">
              ...
              <?php endif; ?>
              <form method="post" action="<?= route_to('vendor_notif_delete', $n['id']) ?>"
    ...
    <div class="px-4 py-3 border-t flex justify-between items-center">
      <form method="post" action="<?= route_to('vendor_notif_mark_all') ?>">
        <?= csrf_field() ?>
        <button type="submit" class="px-3 py-1 rounded bg-blue-600 text-white text-sm hover:bg-blue-700">
          Tandai Semua Dibaca
        </button>
      </form>
      <form method="post" action="<?= route_to('vendor_notif_delete_all') ?>"
