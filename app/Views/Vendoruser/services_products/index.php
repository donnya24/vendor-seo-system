<main class="app-main flex-1 p-4 sm:p-6 lg:p-8 bg-gray-50">
  ...
  <div class="mx-auto w-full max-w-screen-2xl space-y-4">
    ...
    <div class="bg-white ring-1 ring-gray-200 rounded-xl shadow-sm px-5 py-4 sm:px-6 sm:py-5">
      <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        ...
        <div class="flex gap-2">
          <button
            onclick="openModal('<?= route_to('sp_create_group') ?>')"
    ...
    <!-- Tabel -->
    <form id="bulkDeleteForm" method="post" action="<?= route_to('sp_delete_multiple') ?>">
      <?= csrf_field() ?>
      ...
      <div class="bg-white rounded-2xl shadow-sm ring-1 ring-gray-200 overflow-hidden">
        <div class="max-h-[70vh] overflow-auto scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100 scrollbar-thumb-rounded-full">
          <table class="min-w-full text-[13px] sm:text-sm text-gray-800">
            ...
            <?php
            $groupedData = [];
            if (!empty($servicesProducts)) {
              foreach ($servicesProducts as $sp) {
                ...
                if ($serviceName !== '-') {
                  ...
                  if (!empty($sp['product_name'])) {
                    $groupedData[$serviceName]['products'][] = [
                      'id'                  => $sp['id'] ?? null,
                      'product_name'        => $sp['product_name'],
                      'product_description' => $sp['product_description'] ?? '-',
                      'price'               => $sp['price'] ?? 0,
                      'attachment'          => $sp['attachment'] ?? null,
                      'attachment_url'      => $sp['attachment_url'] ?? null,
                    ];
                  }
                }
              }
            ...
            <?php if (!empty($groupedData)): ?>
              <?php foreach ($groupedData as $serviceName => $serviceData): ?>
                <?php
                  ...
                  $editUrl      = route_to('sp_edit_group') . '?service_name=' . rawurlencode($serviceName);
                ...
                <tr class="border-b border-gray-200 hover:bg-blue-50/50 transition-colors">
                  ...
                  <?php if ($productCount > 0): ?>
                    <?php foreach ($serviceData['products'] as $index => $product): ?>
                      ...
                      ?>
                      <?php if ($index > 0): ?></tr><tr class="border-b border-gray-200 hover:bg-blue-50/50 transition-colors"><?php endif; ?>
            ...
            <?php else: ?>
              <tr>
                <td colspan="9" class="px-4 py-6 text-gray-500 text-center">Belum ada layanan / produk.</td>
              </tr>
            <?php endif; ?>
            </tbody>
          </table>
...
/* Hitung index produk berikutnya dari semua field bernama products[IDX] */
function nextProductIndex(){
  let max = -1;
  $root().querySelectorAll('input[name^="products["], textarea[name^="products["], select[name^="products["]')
    .forEach(el => {
...
function initFormScripts(){
  ...
  const editForm = $root().querySelector('form[action="<?= route_to('sp_update_group') ?>"]') 
                || document.querySelector('form[action="<?= route_to('sp_update_group') ?>"]');
...
window.addNewProduct = function(){
  ...
  // Inisialisasi formatting untuk input harga yang baru dibuat
...
document.addEventListener("DOMContentLoaded", function(){
  ...
  function buildServiceGroups(){
    ...
    while(i<rows.length){
      const tr = rows[i];
      const svcTd = tr.querySelector('td:nth-child(2)[rowspan]');
      if(!svcTd){ i++; continue; }
      const rowspan = parseInt(svcTd.getAttribute('rowspan')||'1',10)||1;
      const chunk = rows.slice(i, i+rowspan);
      const productCells = chunk.map((r, idx)=> idx===0 ? r.querySelector('td:nth-child(4)') : r.querySelector('td:nth-child(1)'));
      groups.push({ serviceName:norm(svcTd.textContent), rows:chunk, productCells });
      i += rowspan;
    }
    return groups;
  }
  ...
  function applyFilters(){
    ...
    serviceGroups.forEach(g=>{
      ...
      const show = svcMatch && prdMatch;
      g.rows.forEach(r => r.style.display = show ? '' : 'none');
    });
  }
  function clearFilters(){ if($svc) $svc.value=''; if($prd) $prd.value=''; applyFilters(); }
