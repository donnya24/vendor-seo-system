...
<?php if (!$isModal): ?>
  <div class="flex-1 md:ml-64 p-4">
    <h2 class="text-xl font-semibold mb-4"><?= esc($title ?? 'Tambah Area Layanan') ?></h2>
<?php endif; ?>
...
<div
  x-data="{
    ...
    clearAll(){
      if (!window.Swal) {
        this.allIndonesia = false; this.selected = []; this.query = ''; this.closeDropdown(); return;
      }
      const swalMini = {
      ...
      Swal.fire({
        icon: 'warning', title: 'Hapus semua area?', text: 'Ini akan mengosongkan daftar area di form.',
        showCancelButton: true, confirmButtonText: 'Ya, hapus', cancelButtonText: 'Batal', width: 300, customClass: swalMini
      }).then(r => {
        if (r.isConfirmed) {
          this.allIndonesia = false; this.selected = []; this.query = ''; this.closeDropdown();
          Swal.fire({ icon:'success', title:'Bersih', text:'Semua area dihapus dari form.', timer:1400, showConfirmButton:false, width:280, customClass: swalMini });
        }
      });
    },

    init(){
      // Prefill: tampilkan area yang sudah tersimpan (jika ada)
    ...
    fetchSuggest(){
      ...
      fetch(window.__AREAS_SEARCH_URL + '?q=' + encodeURIComponent(q), { headers: {'X-Requested-With':'XMLHttpRequest'} })
        ...
        .then(res => {
          ...
          this.open = true; this.hi = this.suggestions.length ? 0 : -1;
        })
    ...
    // submit AJAX jika modal
    async handleSubmit(e){
      const fd = new FormData(e.target);
      try{
        const r  = await fetch(e.target.action, { method: 'POST', headers: {'X-Requested-With':'XMLHttpRequest'}, body: fd });
        ...
        if (ct.includes('application/json')) {
          ...
          if (typeof showMini === 'function') {
            ...
            Swal.fire({ icon: (j.status==='success'?'success':'error'), title: (j.status==='success'?'Berhasil':'Info'), text: j.message||'', timer: 1600, showConfirmButton:false });
          }
          if (!j.redirect && typeof closeAreasPopup === 'function') closeAreasPopup();
        } else {
          // bukan JSON → fallback submit normal
          e.target.submit();
        }
      }catch(_){
        if (typeof showMini === 'function') showMini('error', 'Koneksi gagal. Coba lagi.');
        else if (window.Swal) Swal.fire({ icon:'error', title:'Error', text:'Koneksi gagal' });
      }
    }
  }"
  x-init="init()"
