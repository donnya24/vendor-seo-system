<form method="post" action="<?= site_url('admin/services/store') ?>" enctype="multipart/form-data">
    <?= csrf_field() ?>
    ...
    <div class="mb-4">
        ...
        <select class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md" id="vendor_id" name="vendor_id" required>
            ...
            <?php foreach ($vendors as $vendor): ?>
                <option value="<?= $vendor['id'] ?>"><?= esc($vendor['business_name']) ?></option>
            <?php endforeach; ?>
        </select>
    </div>
    ...
    <div class="mb-4">
        ...
        <input type="text" class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" id="service_name" name="service_name" required>
    </div>
    
    <div class="mb-4">
        <label for="service_description" class="block text-sm font-medium text-gray-700">Deskripsi Layanan</label>
        <textarea class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" id="service_description" name="service_description" rows="3"></textarea>
    </div>
            ...
            <div class="product-item border border-gray-300 rounded-md p-3 mb-3">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" name="products[0][product_name]" required>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Harga</label>
                        <input type="number" class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" name="products[0][price]" step="0.01" min="0" required>
                    </div>
                </div>
                <div class="mt-4">
                    <label class="block text-sm font-medium text-gray-700">Deskripsi Produk</label>
                    <textarea class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" name="products[0][product_description]" rows="2"></textarea>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Lampiran</label>
                        <input type="file" class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" name="attachment_0" accept=".pdf,.jpg,.jpeg,.png">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700">URL Lampiran</label>
                        <input type="url" class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" name="products[0][attachment_url]">
                    </div>
                </div>
                <input type="hidden" name="products[0][temp_id]" value="0">
            </div>
        </div>
