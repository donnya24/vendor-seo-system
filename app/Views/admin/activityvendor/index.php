...
<main class="app-main flex-1 p-4 bg-gray-50">
  <div class="bg-white rounded-2xl p-6 shadow">
    ...
    <h2 class="text-xl font-semibold mb-6 text-center">Riwayat Aktivitas Vendor</h2>
    ...
    <div class="mb-5 flex flex-col sm:flex-row gap-4 items-center justify-between">
      ...
      <!-- DELETE ALL BUTTON -->
      <?php if (!empty($logs)): ?>
        <form method="post" action="<?= site_url('admin/activities/vendor/delete-all' . (!empty($vendor_id) ? '?vendor_id=' . $vendor_id : '')) ?>" 
              id="deleteAllForm">
          <?= csrf_field() ?>
        ...
        <table class="min-w-full text-sm">
          <thead class="bg-blue-600 text-white text-xs uppercase tracking-wide sticky top-0 z-10">
            <tr>
              ...
              <th class="p-3 text-center">Vendor</th>
              ...
              <?php foreach($logs as $i => $log): ?>
                <tr class="hover:bg-gray-50">
                  ...
                  <td class="p-3 text-center">
                    <div class="font-medium"><?= esc($log['business_name'] ?? '-') ?></div>
                    <div class="text-xs text-gray-500">ID: <?= esc($log['vendor_id'] ?? '-') ?></div>
                  </td>
              ...
              <tr>
                <td colspan="7" class="p-4 text-center text-gray-500">
                  <div class="py-8">
                    ...
                    <p class="text-lg">Belum ada aktivitas vendor.</p>
                    <?php if (!empty($vendor_id)): ?>
                      <p class="text-sm text-gray-500 mt-1">Vendor ini belum memiliki riwayat aktivitas.</p>
                    <?php endif; ?>
                  </div>
                </td>
              </tr>
    ...
    <div class="mt-3 text-sm text-gray-500 text-left">
      ...
      Catatan: Riwayat ini menampilkan semua aktivitas <strong>Vendor</strong>, bisa difilter berdasarkan vendor tertentu.
    </div>
  </div>
</main>
    ...
    if (deleteForm) {
        deleteForm.addEventListener('submit', function(e) {
            ...
            Swal.fire({
                ...
                html: `Anda akan menghapus <strong>semua riwayat aktivitas${vendorFilter}</strong>.<br><br>Tindakan ini <strong>tidak dapat dibatalkan</strong>!`,
                icon: 'warning',
