<?php
 ...
 // DEBUG di PHP
 // echo "<pre>PHP - Selected Areas: "; print_r($pre); echo "</pre>";
 // echo "<pre>PHP - Vendor: "; print_r($vendor); echo "</pre>";
?>
<?php if (!$isModal): ?>
...
<script>
// DEBUG: Pastikan data dari PHP tersedia di JavaScript
console.log('=== DEBUG AREAS DATA ===');
console.log('PHP Vendor:', <?= json_encode($vendor) ?>);
console.log('PHP Selected Areas:', <?= json_encode($pre) ?>);
console.log('PHP isAll:', <?= $isAll ? 'true' : 'false' ?>);
...
function showMini(status, message, redirect = null) {
    ...
    Swal.fire(config).then((result) => {
        if (redirect && (status === 'success' || result.isConfirmed)) {
            if (typeof closeAreasPopup === 'function') {
                closeAreasPopup();
            } else {
...
<div class="<?= !$isModal ? 'bg-white rounded-lg shadow-sm border border-gray-200 p-6' : '' ?>">
    <div
        x-data="{
            ...
            searchUrl: '<?= site_url('admin/areas/search') ?>',
            ...
            init(){
                ...
                console.log('Pre data from PHP:', window.__AREAS_PRE);
                console.log('All Indonesia:', this.allIndonesia);
                
                // LOAD DATA DARI PHP - INI YANG PENTING!
                if (this.allIndonesia) {
                    ...
                    // PASTIKAN data dari PHP dimasukkan dengan format yang benar
                    this.selected = window.__AREAS_PRE.map(area => ({
                        id: parseInt(area.id),
                        name: area.name,
                        type: area.type,
                ...
                console.log('Selected after init:', this.selected);
                ...
                this.$watch('allIndonesia', (v) => {
                    if (v){
                        ...
                        // Ketika checkbox dinonaktifkan, kembalikan area yang sebelumnya dipilih
                        this.selected = window.__AREAS_PRE.map(area => ({
                            id: parseInt(area.id),
                            name: area.name,
                            type: area.type,
    ...
    >
        ...
        >
            ...
            <div class="space-y-2" x-show="!allIndonesia">
                ...
                <div class="relative">
                    <div class="border border-gray-300 rounded-lg p-3 min-h-[60px] bg-white flex flex-wrap gap-2 items-center transition-colors hover:border-gray-400 focus-within:border-blue-500 focus-within:ring-2 focus-within:ring-blue-200"
                        ...
                        <!-- CHIPS: Area yang sudah dipilih - TAMPIL DI INPUT SEARCH JUGA -->
            ...
            <div class="flex justify-end gap-3 pt-4 border-t border-gray-200">
                <?php if ($isModal): ?>
                    <button type="button" onclick="closeAreasPopup()" class="px-4 py-2.5 rounded-lg border border-gray-300 text-gray-700 hover:bg-gray-50 transition-colors">Tutup</button>
                <?php else: ?>
                    <a href="<?= site_url('admin/areas') ?>" class="px-4 py-2.5 rounded-lg border border-gray-300 text-gray-700 hover:bg-gray-50 transition-colors">Batal</a>
                <?php endif; ?>
