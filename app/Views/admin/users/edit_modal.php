<?php
// Tentukan modal mana yang akan ditampilkan
$modalType = $role === 'vendor' ? 'vendor' : 'seo';
$pageKey = $user['id'] . '-' . $modalType;
$actionSEO = site_url('admin/users/'.$user['id'].'/update?role=seoteam');
$actionVEN = site_url('admin/users/'.$user['id'].'/update?role=vendor');
$seoFullname = $role === 'seoteam' ? ($user['name'] ?? '') : '';
$seoPhone = $role === 'seoteam' ? ($user['phone'] ?? '') : '';
$seoEmail = $role === 'seoteam' ? ($user['email'] ?? '') : '';
$usernameSEO = ''; // SEO: paksa isi username baru
$venFullname = $role === 'vendor' ? ($user['fullname'] ?? $user['name'] ?? '') : '';
$venPhone = $role === 'vendor' ? ($user['phone'] ?? '') : '';
$venEmail = $role === 'vendor' ? ($user['email'] ?? '') : '';
$usernameVEN = $role === 'vendor' ? ($user['username'] ?? '') : '';

// Data vendor profile
$vendorStatus = 'active';
$vendorIsVerified = false;
$commissionRate = null;
if ($role === 'vendor' && isset($vendorProfile)) {
    $vendorStatus = strtolower($vendorProfile['status'] ?? $vendorProfile['vendor_status'] ?? 'active');
    $vendorIsVerified = (int)($vendorProfile['is_verified'] ?? 0) === 1;
    $commissionRate = is_numeric($vendorProfile['commission_rate'] ?? null) ? (float)$vendorProfile['commission_rate'] : null;
}
?>

<!-- Modal SEO -->
<?php if ($role === 'seoteam'): ?>
<div id="seoModal" class="fixed inset-0 z-[999] flex items-start justify-center p-3 sm:p-4"
...
<!-- Modal Vendor -->
<?php if ($role === 'vendor'): ?>
<div id="vendorModal" class="fixed inset-0 z-[999] flex items-start justify-center p-3 sm:p-4"
...
function editUsersPage(initialType, pageKey) {
    return {
        ...
        init(){
            this.$nextTick(() => { this.modalType = '<?= $modalType ?>'; this.open = true; });
            ...
            window.addEventListener('popstate', () => {
                if (this.open) {
                    this.open = false;
                    document.body.style.overflow = '';
                    setTimeout(() => window.location.replace("<?= site_url('admin/users') ?>"), 80);
                }
            }, { once: true });
        }
    }
